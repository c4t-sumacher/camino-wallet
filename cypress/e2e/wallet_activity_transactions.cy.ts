...
describe('activity transactions', () => {
    ...
    it('has access/activity transactions', () => {
        ...
        cy.get('.tx_table', { timeout: 7000 }).should('be.visible')
        cy.get('.tx_cols', { timeout: 7000 }).should('be.visible')
        console.log("access data: ", dataBody);
        cy.log("Table Ok");
        cy.get('.meta_col > div > .time', { timeout: 7000 }).invoke("text").then((text) => {
            ...
            console.log("str4lformatDate", dateUTC)
            cy.get('[data-v-e579b3ee=""] > .utxos > :nth-child(1) > .tx_out > .amount').should("be.visible").invoke("text")
                .then(textAmount => {
                    let textAmountArr = textAmount.split(" ").filter(textData =>
                        textData != "\n" && textData != "" && textData != "CAM\n"
                    )
                    let amount = parseInt(textAmountArr[0].replace("\n", "")) * 1000000000
                    if (dateUTC.replace(".000", "") == dataBody.transactions[0].outputs[1].timestamp || amount == parseInt(dataBody.transactions[0].outputs[1].amount)) {
                        cy.log("success")
                    } else {
                        cy.log("failed")
                    }
                })
            console.log("textDate", textDate.toString().split(","));
        });
