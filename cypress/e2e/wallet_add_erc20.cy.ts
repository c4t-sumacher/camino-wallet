...
describe('activity transactions', () => {
    ...
    it('has access/add erc20', async () => {
        ...
        cy.get('.add_token_body > :nth-child(1) > input')
            ...
            cy.get('.erc_row > .balance_col').invoke('text').then((text) => {
                cy.log(text)
            ...
            cy.get('[data-cy="token-erc20-name"]').invoke("text").then(nameERC20 => {
                cy.get('[data-cy="token-erc20-symbol"]').invoke("text").then(symbolERC20 => {
